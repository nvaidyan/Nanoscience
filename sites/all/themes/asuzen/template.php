  ...
  */
  ...
  if (!$user->uid) {
    if (module_exists('cas')) {
      $items[] = l(t('SIGN IN'), 'cas');
    }
    else if (module_exists('asu_webauth')) {
      // Check webauth module version
      if (ASU_WEBAUTH_MODULE_API_VERSION >= 2) {
        $items[] = l(t('SIGN IN'), 'asuwebauth-login');
      }
      else {
        $items[] = l(t('SIGN IN'), 'user', array(), 'destination='.$_REQUEST['q']);
      }
    }
    else {
      $items[] = l(t('SIGN IN'), 'user');
    }
  }
  // Authenticated
  else {
    ...
    if (module_exists('cas')) {
      $items[] = l(t('SIGN OUT'), 'caslogout');
    }
    else if (module_exists('asu_webauth') && ASU_WEBAUTH_MODULE_API_VERSION >= 2) {
        $items[] = l(t('SIGN OUT'), 'asuwebauth-logout');
    }
    else {
      $items[] = l(t('SIGN OUT'), 'logout');
    }
  }
  $vars['header_nav'] = theme('item_list', $items, NULL, 'ul', array('id' => $ul_id));

  /************** ~-ASUZEN 3.0 additions-~ *********************/

  /**
   * #1 Correct information about the presence of a
   * left sidebar in body_classes
   */
  $classes = explode(' ', $vars['body_classes']);
  // Unset no-sidebars array element
  if ($index = array_search('no-sidebars', $classes)) {
    unset($classes[$index]);
  }

  $true_left = $vars['logo'] || $vars['site_name'] || $vars['site_slogan'];
  if ($vars['layout'] == 'none' && !$true_left) {
    $classes[] = 'no-sidebars';
  }
  elseif ($vars['layout'] == 'none' && $true_left) {
  ...
  if (theme_get_setting('asuzen_asu_header')) {
     $vars['asu_header_path'] = theme_get_setting('asuzen_asu_header');
  }
  else {
     $vars['asu_header_path'] = '/afs/asu.edu/www/asuthemes/3.0/headers/default.html';
  }
